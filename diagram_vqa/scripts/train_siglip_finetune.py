...
def set_trainable(model: torch.nn.Module, mode: str, blocks: int) -> list[torch.nn.Parameter]:
    ...
    if mode == "last-blocks":
        modules = [
            *list(model.vision_model.encoder.layers[-blocks:]),
            model.vision_model.post_layernorm,
            *list(model.text_model.encoder.layers[-blocks:]),
    ...
    if not parameters:
        raise RuntimeError("No SigLIP parameters were selected for fine-tuning")
    return parameters


def text_inputs(tokenizer: Any, values: list[str], device: torch.device) -> dict[str, torch.Tensor]:
    tokens = tokenizer(
        values,
        return_tensors="pt",
        padding="max_length",
...
def encode_rows(
    ...
    with torch.inference_mode():
        for start in tqdm(range(0, len(documents), batch_size), desc="encode images", leave=False):
            ...
            encoded = F.normalize(model.get_image_features(pixel_values=pixels), dim=-1)
            image_features.append(encoded.float().cpu())
        for start in tqdm(range(0, len(rows), batch_size * 2), desc="encode questions", leave=False):
            values = [str(row["question"]) for row in rows[start : start + batch_size * 2]]
            encoded = F.normalize(model.get_text_features(**text_inputs(tokenizer, values, device)), dim=-1)
...
def evaluate(
    ...
    images, document_ids, questions, question_ids = encode_rows(
        model, processor, tokenizer, rows, device, batch_size
    )
    return retrieval_metrics(questions @ images.T, question_ids, document_ids)


def main() -> None:
    parser = argparse.ArgumentParser(description="Fine-tune SigLIP2 for diagram retrieval")
    parser.add_argument("--dataset", choices=DATASETS, required=True)
    parser.add_argument("--seed", type=int, choices=SEEDS, default=42)
    parser.add_argument("--mode", choices=["last-blocks", "full"], default="full")
    ...
    parser.add_argument("--output-dir", type=Path, default=ROOT / "runs/siglip_finetune")
    ...
    ensure_free_space(run_dir, minimum_free_gb=args.min_free_gb)
    run_dir.mkdir(parents=True, exist_ok=True)
    config = vars(args) | {"output_dir": str(args.output_dir), "device": str(device), "backbone": BACKBONE}
    ...
    model = AutoModel.from_pretrained(BACKBONE, **load_kwargs)
    if device.type != "cuda":
        model.to(device)
    processor = AutoImageProcessor.from_pretrained(BACKBONE, use_fast=False)
    ...
    dataset = SiglipRows(train_rows, processor)
    ...
    for epoch in range(start_epoch, args.epochs + 1):
        ...
        losses: list[float] = []
        for step, batch in enumerate(tqdm(loader, desc=f"{args.dataset} SigLIP epoch {epoch}/{args.epochs}"), start=1):
            pixels = batch["images"].to(device, non_blocking=True)
            tokens = text_inputs(tokenizer, batch["questions"], device)
            with torch.autocast(device_type=device.type, dtype=dtype, enabled=device.type == "cuda"):
                image_features = F.normalize(model.get_image_features(pixel_values=pixels), dim=-1)
                text_features = F.normalize(model.get_text_features(**tokens), dim=-1)
        ...
        val = evaluate(model, processor, tokenizer, val_rows, device, args.eval_batch_size)
        ...
        torch.save({"model": state, "epoch": epoch, "history": history}, last_path)
        if mean_r1 > best_r1:
            best_r1 = mean_r1
            stale = 0
    ...
    test = evaluate(model, processor, tokenizer, test_rows, device, args.eval_batch_size)
    elapsed = time.perf_counter() - started
    metrics = {
        "schema_version": 1,
        "dataset": args.dataset,
        "model": "siglip2_finetuned",
        "backbone": BACKBONE,
